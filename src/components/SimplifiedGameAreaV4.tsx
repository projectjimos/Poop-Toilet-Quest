...
function drawMinimap(ctx: CanvasRenderingContext2D, runtime: Runtime, width: number, toilet: Toilet) {
  ...
  ctx.fillStyle = 'rgba(2, 6, 23, 0.88)';
  ctx.strokeStyle = enemiesInRange.length ? '#fbbf24' : dangerEnemies.length ? '#fb7185' : '#38bdf8';
  ctx.lineWidth = 2;
  ctx.beginPath();
  ...
  for (const enemy of runtime.enemies) {
    ...
    ctx.beginPath();
    ctx.arc(mapX(enemy.x), mapY(enemy.y), inRange ? 4.2 : close ? 3.6 : 2.8, 0, Math.PI * 2);
    ctx.fill();
  }
  ...
  ctx.textAlign = 'right';
  ctx.fillStyle = enemiesInRange.length ? '#fbbf24' : dangerEnemies.length ? '#fb7185' : '#67e8f9';
  ctx.fillText(`${enemiesInRange.length} IN RANGE`, x + mapSize - 9, y + 13);

  const bannerY = y + mapSize + 6;
  ctx.fillStyle = enemiesInRange.length ? 'rgba(251, 191, 36, 0.94)' : dangerEnemies.length ? 'rgba(251, 113, 133, 0.92)' : 'rgba(8, 47, 73, 0.92)';
  ctx.beginPath();
  ctx.roundRect(x, bannerY, mapSize, 23, 9);
  ctx.fill();
  ctx.fillStyle = enemiesInRange.length ? '#1e293b' : '#e0f2fe';
  ctx.font = '900 10px monospace';
  ctx.textAlign = 'center';
  ctx.fillText(enemiesInRange.length ? 'FLUSH NOW!' : dangerEnemies.length ? 'ENEMY CLOSE!' : 'AREA CLEAR', x + mapSize / 2, bannerY + 15);
  ctx.restore();
}

export default function SimplifiedGameAreaV4({
  coins,
  addCoins,
  unlockedToilets,
  unlockToilet,
  ...
  useEffect(() => {
    ...
    const drawFrame = (now: number) => {
      ...
      if (runtime.active && !isLoopPaused) {
        ...
        spawnEnemy(now);
        if (runtime.coins.length < 10) runtime.coins.push(randomCoin());
        ...
        for (const enemy of runtime.enemies) {
          if (enemy.hp <= 0) {
            ...
            pushParticle(`+${enemy.scoreValue} kill`, enemy.x, enemy.y - 36, '#a7f3d0');
      ...
      for (const enemy of runtimeForDraw.enemies) {
        ...
        ctx.fillText(`${coinDropFor(enemy, runtimeForDraw.wave)}🪙`, enemy.x, enemy.y - enemy.size - 22);
      }
    ...
    <div className="overflow-hidden rounded-[2rem] border border-slate-800 bg-slate-950/80 shadow-2xl shadow-black/30">
      <div className="flex flex-col gap-3 border-b border-slate-800 bg-slate-900/70 p-4 font-mono lg:flex-row lg:items-center lg:justify-between">
        <div>
          ...
          <p className="mt-1 text-xs font-bold text-slate-400">Pause freezes enemies, damage, spawning, cooldowns, coins, fruit, and movement.</p>
        </div>
      ...
      <div className="grid gap-0 lg:grid-cols-[minmax(0,1fr)_330px]">
        <div ref={shellRef} className="relative min-h-[520px] overflow-hidden bg-slate-950">
          ...
          {gameState === 'playing' && (
            <button
              type="button"
              onClick={togglePause}
              className="absolute left-4 top-4 z-30 rounded-2xl border border-sky-300/35 bg-slate-950/80 px-4 py-2 font-mono text-xs font-black uppercase text-sky-100 shadow-xl backdrop-blur-md transition hover:bg-sky-400/15"
            >
          ...
          {gameState !== 'playing' && (
            <div className="absolute inset-0 z-20 flex items-center justify-center bg-slate-950/75 p-5 text-center font-mono backdrop-blur-sm">
              <div className="w-full max-w-md rounded-3xl border border-amber-400/25 bg-slate-900/95 p-6 shadow-2xl">
                ...
                <p className="mt-2 text-sm font-bold text-slate-400">{gameState === 'gameover' ? `You scored ${score} kills and collected ${sessionCoins} coins this run.` : 'Press P or Esc during a run to pause.'}</p>
        ...
        <aside className="border-t border-slate-800 bg-slate-900/80 p-4 font-mono lg:border-l lg:border-t-0">
          ...
          <div className="mt-4 rounded-2xl border border-slate-800 bg-slate-950/70 p-4 text-xs font-bold leading-relaxed text-slate-400">
            <div className="text-[10px] font-black uppercase tracking-[0.22em] text-sky-300">Pause</div>
            <p className="mt-2">Use the Pause button, P, or Esc. Pausing freezes the entire run until you resume.</p>
          </div>

          <div className="mt-4 rounded-2xl border border-slate-800 bg-slate-950/70 p-4 text-xs font-bold leading-relaxed text-slate-400">
            <div className="text-[10px] font-black uppercase tracking-[0.22em] text-amber-300">Coin Drops</div>
            <div className="mt-2 grid gap-1"><span>🦠/🪰: 1 coin</span><span>🧼: 2 coins</span><span>🧻: 3 coins</span><span>🧹: 4+ coins</span><span className="text-amber-200">Later waves add bonus coins.</span></div>
          </div>

          <div className="mt-4 rounded-2xl border border-slate-800 bg-slate-950/70 p-4 text-xs font-bold leading-relaxed text-slate-400">
            <div className="text-[10px] font-black uppercase tracking-[0.22em] text-amber-300">Map Legend</div>
            <div className="mt-2 grid gap-1"><span><span className="text-emerald-300">●</span> You</span><span><span className="text-rose-400">●</span> Enemy</span><span><span className="text-amber-300">●</span> Enemy inside flush range</span><span><span className="text-yellow-300">●</span> Coins · <span className="text-emerald-300">●</span> Fruit</span></div>
          </div>

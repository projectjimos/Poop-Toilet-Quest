...
function randomCoin(worldSize = WORLD_SIZE, value = 1): Coin {
  return {
    id: id('coin'),
...
function coinNear(x: number, y: number, worldSize = WORLD_SIZE, value = 1): Coin {
  const angle = Math.random() * Math.PI * 2;
  const radius = 18 + Math.random() * 44;
  return {
    id: id('coin'),
    x: clamp(x + Math.cos(angle) * radius, 45, worldSize - 45),
    y: clamp(y + Math.sin(angle) * radius, 45, worldSize - 45),
    size: value > 1 ? 18 : 15,
    value,
    wobble: Math.random() * Math.PI * 2,
  };
...
function drawMinimap(ctx: CanvasRenderingContext2D, runtime: Runtime, width: number, toilet: Toilet) {
  ...
  const enemiesInRange = runtime.enemies.filter((enemy) => dist(player.x, player.y, enemy.x, enemy.y) <= flushRange + enemy.size);
  const dangerEnemies = runtime.enemies.filter((enemy) => dist(player.x, player.y, enemy.x, enemy.y) <= 260 + enemy.size);
  const mapX = (worldX: number) => innerX + (worldX / runtime.worldSize) * innerW;
  const mapY = (worldY: number) => innerY + (worldY / runtime.worldSize) * innerH;
  const px = mapX(player.x);
  const py = mapY(player.y);
  const rangeRadius = Math.max(8, (flushRange / runtime.worldSize) * innerW);
  ...
  for (const enemy of runtime.enemies) {
    const inRange = dist(player.x, player.y, enemy.x, enemy.y) <= flushRange + enemy.size;
    const close = dist(player.x, player.y, enemy.x, enemy.y) <= 260 + enemy.size;
...
export default function SimplifiedGameAreaV2({
  coins,
  addCoins,
  unlockedToilets,
  unlockToilet,
  ...
  const pushParticle = useCallback((text: string, x: number, y: number, color = '#fbbf24') => {
    ...
    runtime.particles.push({ id: id('fx'), x, y, vx: (Math.random() - 0.5) * 40, vy: -55 - Math.random() * 35, life: 0, maxLife: 650, text, color });
  ...
  const triggerFlush = useCallback(() => {
    ...
    for (const enemy of runtime.enemies) {
      if (dist(runtime.player.x, runtime.player.y, enemy.x, enemy.y) <= toilet.flushRadius + enemy.size) {
  ...
  useEffect(() => {
    ...
    const drawFrame = (now: number) => {
      ...
      if (runtime.active) {
        ...
        spawnEnemy(now);
        if (runtime.coins.length < 10) runtime.coins.push(randomCoin(runtime.worldSize));
        ...
        if (now - runtime.lastFruitRollMs > 1000) {
          ...
          if (runtime.fruits.length < MAX_FRUITS && Math.random() < 0.025) {
            ...
            runtime.fruits.push({ id: id('fruit'), x: 80 + Math.random() * (runtime.worldSize - 160), y: 80 + Math.random() * (runtime.worldSize - 160), emoji: fruit.emoji, heal: fruit.heal, size: fruit.size, wobble: Math.random() * Math.PI * 2 });
          }
        }
        ...
        for (const enemy of runtime.enemies) {
          if (enemy.hp <= 0) {
            ...
            for (let i = 0; i < droppedCoins; i++) {
              runtime.coins.push(coinNear(enemy.x, enemy.y, runtime.worldSize, 1));
            }
          } else {
        ...
        runtime.coins = runtime.coins.filter((coin) => {
          if (dist(player.x, player.y, coin.x, coin.y) < player.size + coin.size) {
        ...
        runtime.fruits = runtime.fruits.filter((fruit) => {
          if (dist(player.x, player.y, fruit.x, fruit.y) < player.size + fruit.size) {
    ...
    <div className="rounded-[2rem] border border-slate-800 bg-slate-950/80 shadow-2xl shadow-black/30 overflow-hidden">
      <div className="flex flex-col gap-3 border-b border-slate-800 bg-slate-900/70 p-4 font-mono lg:flex-row lg:items-center lg:justify-between">
        <div>
          ...
          <p className="mt-1 text-xs font-bold text-slate-400">Stronger enemies now drop more coins. Watch the coin count above enemy heads.</p>
        </div>
          ...
          {gameState !== 'playing' && (
            <div className="absolute inset-0 z-20 flex items-center justify-center bg-slate-950/75 p-5 text-center font-mono backdrop-blur-sm">
              <div className="w-full max-w-md rounded-3xl border border-amber-400/25 bg-slate-900/95 p-6 shadow-2xl">
                ...
                <p className="mt-2 text-sm font-bold text-slate-400">{gameState === 'gameover' ? `You scored ${score} kills and collected ${sessionCoins} coins this run.` : 'Stronger enemies drop more coins. Use the minimap to time flushes.'}</p>
        ...
        <aside className="border-t border-slate-800 bg-slate-900/80 p-4 font-mono lg:border-l lg:border-t-0">
          ...
          <div className="mt-4 rounded-2xl border border-slate-800 bg-slate-950/70 p-4 text-xs font-bold leading-relaxed text-slate-400">
            ...
            <div className="mt-2 grid gap-1">
              <span>🦠 Tiny Germ / 🪰 Fast Fly: 1 coin</span>
              <span>🧼 Soap Guard: 2 coins</span>
              <span>🧻 Paper Tank: 3 coins</span>
              <span>🧹 Brush Brute: 4+ coins</span>
              <span className="text-amber-200">Later waves add bonus coins.</span>
            </div>
          </div>

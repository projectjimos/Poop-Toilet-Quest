...
function flushReadyRatio(runtime: Runtime, toilet: Toilet) {
  if (!runtime.active) return 1;
  if (toilet.cooldownMs <= 0) return 1;
  return clamp(1 - runtime.flushCooldownMs / toilet.cooldownMs, 0, 1);
}

function drawPlayerFlushBar(ctx: CanvasRenderingContext2D, player: Player, runtime: Runtime, toilet: Toilet) {
...
function drawMinimap(ctx: CanvasRenderingContext2D, runtime: Runtime, width: number, toilet: Toilet) {
  ...
  const flushRange = toilet.flushRadius;
  const enemiesInRange = runtime.enemies.filter((enemy) => distance(player.x, player.y, enemy.x, enemy.y) <= flushRange + enemy.size);
  ...
  const rangeRadius = Math.max(8, (flushRange / WORLD_SIZE) * innerW);
  ...
  for (const enemy of runtime.enemies) {
    const inRange = distance(player.x, player.y, enemy.x, enemy.y) <= flushRange + enemy.size;
...
export default function SimplifiedGameAreaV3({
  coins,
  addCoins,
  unlockedToilets,
  unlockToilet,
    ...
    <div className="rounded-[2rem] border border-slate-800 bg-slate-950/80 shadow-2xl shadow-black/30 overflow-hidden">
      <div className="flex flex-col gap-3 border-b border-slate-800 bg-slate-900/70 p-4 font-mono lg:flex-row lg:items-center lg:justify-between">
        <div>
          ...
          <p className="mt-1 text-xs font-bold text-slate-400">The blue bar above your poop fills up. Full bar means your flush is ready.</p>
        </div>
      ...
      <div className="grid gap-0 lg:grid-cols-[minmax(0,1fr)_330px]">
          ...
          {gameState !== 'playing' && (
            <div className="absolute inset-0 z-20 flex items-center justify-center bg-slate-950/75 p-5 text-center font-mono backdrop-blur-sm">
              <div className="w-full max-w-md rounded-3xl border border-amber-400/25 bg-slate-900/95 p-6 shadow-2xl">
                ...
                <p className="mt-2 text-sm font-bold text-slate-400">{gameState === 'gameover' ? `You scored ${score} kills and collected ${sessionCoins} coins this run.` : 'When the blue bar over your character is full, you can flush again.'}</p>
        ...
        <aside className="border-t border-slate-800 bg-slate-900/80 p-4 font-mono lg:border-l lg:border-t-0">
          ...
          <div className="mt-4 rounded-2xl border border-slate-800 bg-slate-950/70 p-4 text-xs font-bold leading-relaxed text-slate-400">
            <div className="text-[10px] font-black uppercase tracking-[0.22em] text-sky-300">Flush Meter</div>
            <p className="mt-2">The blue bar above your poop charges after every flush. While it is not full, Space and the mobile Flush button will not fire.</p>
          </div>
          ...
          {shopOpen && (
            <div className="mt-4 max-h-[440px] space-y-2 overflow-y-auto pr-1">
              {shopToilets.map((toilet) => {
                ...
                return <div key={toilet.id} className="rounded-2xl border border-slate-800 bg-slate-950/85 p-3"><div className="flex items-start justify-between gap-3"><div className="flex min-w-0 gap-3"><div className="text-3xl">{toilet.emoji}</div><div className="min-w-0"><div className="truncate text-xs font-black text-white">{toilet.name}</div><div className="mt-1 text-[10px] font-bold text-slate-500">Cost {toilet.cost} · DMG {toilet.damage} · R {toilet.flushRadius}</div></div></div><button type="button" disabled={!owned && !affordable} onClick={() => buyToilet(toilet)} className={`rounded-lg px-3 py-2 text-[10px] font-black uppercase transition ${active ? 'bg-emerald-400 text-slate-950' : owned ? 'bg-cyan-400/15 text-cyan-200 hover:bg-cyan-400/25' : affordable ? 'bg-amber-400 text-slate-950 hover:bg-amber-300' : 'cursor-not-allowed bg-slate-800 text-slate-500'}`}>{active ? 'Active' : owned ? 'Equip' : affordable ? 'Buy' : 'Need Coins'}</button></div>{owned && toilet.id !== 'porta_potty' && !active && <button type="button" onClick={() => sellOwnedToilet(toilet)} className="mt-2 text-[10px] font-black uppercase text-rose-300 hover:text-rose-200">Sell for {Math.floor(toilet.cost * 0.9)} coins</button>}</div>;
              })}
            </div>
          )}

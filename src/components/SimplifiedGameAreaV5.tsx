...
function randomCoin(value = 1): Coin {
  return {
    id: id('coin'),
    x: 70 + Math.random() * (WORLD_SIZE - 140),
    y: 70 + Math.random() * (WORLD_SIZE - 140),
    value,
    size: value > 1 ? 18 : 15,
...
function coinNear(x: number, y: number, value = 1): Coin {
  ...
  return {
    id: id('coin'),
    x: clamp(x + Math.cos(angle) * radius, 45, WORLD_SIZE - 45),
    y: clamp(y + Math.sin(angle) * radius, 45, WORLD_SIZE - 45),
    value,
    size: value > 1 ? 18 : 15,
    wobble: Math.random() * Math.PI * 2,
  };
...
function makeRuntime(): Runtime {
  return {
    ...
    enemies: [],
    coins: Array.from({ length: 10 }, () => randomCoin()),
    fruits: [],
    particles: [],
    wave: 1,
    score: 0,
...
function hasBoss(runtime: Runtime) {
  return runtime.enemies.some((enemy) => enemy.isBoss);
}

function currentBoss(runtime: Runtime) {
  return runtime.enemies.find((enemy) => enemy.isBoss) || null;
}

function flushReadyRatio(runtime: Runtime, toilet: Toilet) {
  if (!runtime.active || toilet.cooldownMs <= 0) return 1;
  return clamp(1 - runtime.flushCooldownMs / toilet.cooldownMs, 0, 1);
}
...
function drawPlayerFlushBar(ctx: CanvasRenderingContext2D, player: PlayerState, runtime: Runtime, toilet: Toilet) {
  ...
  ctx.beginPath();
  ctx.roundRect(x + 2, y + 2, Math.max(0, (barWidth - 4) * ratio), barHeight - 4, 4);
  ctx.fill();
  ctx.font = '900 9px monospace';
  ctx.textAlign = 'center';
  ctx.textBaseline = 'bottom';
  ctx.fillStyle = ready ? '#e0f2fe' : '#93c5fd';
...
function drawMinimap(ctx: CanvasRenderingContext2D, runtime: Runtime, width: number, toilet: Toilet) {
  ...
  const rangeRadius = Math.max(8, (toilet.flushRadius / WORLD_SIZE) * innerW);
  ...
  for (let i = 1; i < 4; i++) {
    ctx.beginPath();
    ctx.moveTo(innerX + (innerW * i) / 4, innerY);
    ctx.lineTo(innerX + (innerW * i) / 4, innerY + innerH);
    ctx.stroke();
    ctx.beginPath();
    ctx.moveTo(innerX, innerY + (innerH * i) / 4);
    ctx.lineTo(innerX + innerW, innerY + (innerH * i) / 4);
    ctx.stroke();
  }

  ctx.strokeStyle = enemiesInRange.length ? 'rgba(251, 191, 36, 0.75)' : 'rgba(56, 189, 248, 0.42)';
  ctx.lineWidth = 1.5;
  ctx.beginPath();
  ...
  for (const coin of runtime.coins) {
    ctx.fillStyle = coin.value > 1 ? '#f59e0b' : '#facc15';
    ctx.beginPath();
    ctx.arc(mapX(coin.x), mapY(coin.y), coin.value > 1 ? 2.5 : 1.7, 0, Math.PI * 2);
    ctx.fill();
  }
  ...
  for (const enemy of runtime.enemies) {
    const inRange = distance(player.x, player.y, enemy.x, enemy.y) <= toilet.flushRadius + enemy.size;
    const close = distance(player.x, player.y, enemy.x, enemy.y) <= 260 + enemy.size;
    ctx.fillStyle = enemy.isBoss ? '#f97316' : inRange ? '#fbbf24' : close ? '#fb7185' : '#ef4444';
    ctx.beginPath();
    ctx.arc(mapX(enemy.x), mapY(enemy.y), enemy.isBoss ? 6.5 : inRange ? 4.2 : close ? 3.6 : 2.8, 0, Math.PI * 2);
    ctx.fill();
  }
  ...
  ctx.textAlign = 'left';
  ctx.fillText('MINIMAP', x + 10, y + 13);
  ctx.textAlign = 'right';
  ctx.fillStyle = boss ? '#fb923c' : enemiesInRange.length ? '#fbbf24' : dangerEnemies.length ? '#fb7185' : '#67e8f9';
  ctx.fillText(boss ? 'BOSS!' : `${enemiesInRange.length} IN RANGE`, x + mapSize - 9, y + 13);

  const bannerY = y + mapSize + 6;
  ctx.fillStyle = boss ? 'rgba(249, 115, 22, 0.94)' : enemiesInRange.length ? 'rgba(251, 191, 36, 0.94)' : dangerEnemies.length ? 'rgba(251, 113, 133, 0.92)' : 'rgba(8, 47, 73, 0.92)';
  ctx.beginPath();
  ctx.roundRect(x, bannerY, mapSize, 23, 9);
  ctx.fill();
  ctx.fillStyle = boss ? '#111827' : enemiesInRange.length ? '#1e293b' : '#e0f2fe';
  ctx.font = '900 10px monospace';
  ctx.textAlign = 'center';
  ctx.fillText(boss ? 'BOSS WAVE!' : enemiesInRange.length ? 'FLUSH NOW!' : dangerEnemies.length ? 'ENEMY CLOSE!' : 'AREA CLEAR', x + mapSize / 2, bannerY + 15);
  ctx.restore();
}

export default function SimplifiedGameAreaV5({
  coins,
  addCoins,
  unlockedToilets,
  unlockToilet,
  ...
  const spawnBossIfNeeded = useCallback(() => {
    ...
    runtime.enemies.push({
      ...template,
      id: id('boss'),
      x: clamp(player.x + Math.cos(angle) * 520, 80, WORLD_SIZE - 80),
      y: clamp(player.y + Math.sin(angle) * 520, 80, WORLD_SIZE - 80),
      hp: scaledHp,
      maxHp: scaledHp,
      speed: template.speed + runtime.wave * 1.5,
      isBoss: true,
      bossWave: runtime.wave,
    });
  ...
  const spawnEnemy = useCallback((now: number) => {
    ...
    runtime.enemies.push({
      ...template,
      id: id('enemy'),
      x: clamp(player.x + Math.cos(angle) * distFromPlayer, 35, WORLD_SIZE - 35),
      y: clamp(player.y + Math.sin(angle) * distFromPlayer, 35, WORLD_SIZE - 35),
      hp: scaledHp,
      maxHp: scaledHp,
  ...
  const triggerFlush = useCallback(() => {
    ...
    for (const enemy of runtime.enemies) {
      if (distance(runtime.player.x, runtime.player.y, enemy.x, enemy.y) <= toilet.flushRadius + enemy.size) {
        enemy.hp -= enemy.isBoss ? Math.round(toilet.damage * 0.75) : toilet.damage;
        pushParticle(`-${enemy.isBoss ? Math.round(toilet.damage * 0.75) : toilet.damage}`, enemy.x, enemy.y - 20, toilet.pulseColor);
      }
    }
  ...
  useEffect(() => {
    ...
    const drawFrame = (now: number) => {
      ...
      if (runtime.active && !isLoopPaused) {
        ...
        player.x = clamp(player.x + player.vx * dt, player.size, WORLD_SIZE - player.size);
        player.y = clamp(player.y + player.vy * dt, player.size, WORLD_SIZE - player.size);
        ...
        spawnBossIfNeeded();
        spawnEnemy(now);
        if (!hasBoss(runtime) && runtime.coins.length < 10) runtime.coins.push(randomCoin());

        if (now - runtime.lastFruitRollMs > 1000) {
          runtime.lastFruitRollMs = now;
          if (runtime.fruits.length < MAX_FRUITS && Math.random() < (hasBoss(runtime) ? 0.045 : 0.025)) {
            const fruit = FRUITS[Math.floor(Math.random() * FRUITS.length)];
            runtime.fruits.push({ id: id('fruit'), x: 80 + Math.random() * (WORLD_SIZE - 160), y: 80 + Math.random() * (WORLD_SIZE - 160), ...fruit, wobble: Math.random() * Math.PI * 2 });
          }
        }
        ...
        for (const enemy of runtime.enemies) {
          ...
          if (len < player.size + enemy.size * 0.55) {
            player.hp -= (enemy.isBoss ? 35 : 22) * dt;
            if (Math.random() < 0.06) pushParticle(enemy.isBoss ? 'BOSS HIT!' : 'Ouch!', player.x, player.y - 25, '#fb7185');
          }
        }
        ...
        for (const enemy of runtime.enemies) {
          if (enemy.hp <= 0) {
            ...
            if (enemy.isBoss) {
              runtime.bossDefeatedWaves.push(enemy.bossWave || runtime.wave);
              setBossName(null);
              pushParticle(`${enemy.name} defeated!`, enemy.x, enemy.y - 64, '#fb923c');
            }
            pushParticle(`+${droppedCoins} coins`, enemy.x, enemy.y - 18, '#facc15');
            pushParticle(`+${enemy.scoreValue} score`, enemy.x, enemy.y - 36, '#a7f3d0');
            for (let i = 0; i < droppedCoins; i++) runtime.coins.push(coinNear(enemy.x, enemy.y));
          } else {
            aliveEnemies.push(enemy);
          }
        }
        runtime.enemies = aliveEnemies.slice(-MAX_ENEMIES);
        ...
        runtime.coins = runtime.coins.filter((coin) => {
          if (distance(player.x, player.y, coin.x, coin.y) < player.size + coin.size) {
            ...
            addCoins(coin.value);
            pushParticle(`+${coin.value} 🪙`, coin.x, coin.y - 10, '#fcd34d');
        ...
        if (nextWave !== runtime.wave && !hasBoss(runtime)) {
          runtime.wave = nextWave;
          pushParticle(runtime.wave % 5 === 0 ? `Boss wave ${runtime.wave}!` : `Wave ${runtime.wave}`, player.x, player.y - 55, runtime.wave % 5 === 0 ? '#fb923c' : '#fbbf24');
        }
        if (player.hp <= 0) {
          player.hp = 0;
        ...
        runtime.cameraX = clamp(player.x - width / 2, 0, WORLD_SIZE - width);
        runtime.cameraY = clamp(player.y - height / 2, 0, WORLD_SIZE - height);
      ...
      const boss = currentBoss(runtimeForDraw);
      ...
      for (const coin of runtimeForDraw.coins) {
        ...
        ctx.fillText(coin.value > 1 ? '💰' : '🪙', coin.x, coin.y + float);
      }
      ...
      for (const enemy of runtimeForDraw.enemies) {
        if (enemy.isBoss) {
          ...
          ctx.lineWidth = 5;
          ctx.beginPath();
          ctx.arc(enemy.x, enemy.y, enemy.size + 16 + Math.sin(now / 140) * 4, 0, Math.PI * 2);
          ctx.stroke();
          ctx.restore();
        }
        ctx.font = `${enemy.size + (enemy.isBoss ? 18 : 8)}px Arial`;
        ctx.textAlign = 'center';
        ctx.textBaseline = 'middle';
        ctx.fillText(enemy.emoji, enemy.x, enemy.y);
        const barWidth = enemy.isBoss ? enemy.size + 70 : enemy.size + 12;
        const ratio = clamp(enemy.hp / enemy.maxHp, 0, 1);
        ctx.fillStyle = '#334155';
        ctx.fillRect(enemy.x - barWidth / 2, enemy.y - enemy.size - (enemy.isBoss ? 30 : 14), barWidth, enemy.isBoss ? 8 : 4);
        ctx.fillStyle = enemy.isBoss ? '#fb923c' : ratio > 0.5 ? '#22c55e' : ratio > 0.25 ? '#f59e0b' : '#ef4444';
        ctx.fillRect(enemy.x - barWidth / 2, enemy.y - enemy.size - (enemy.isBoss ? 30 : 14), barWidth * ratio, enemy.isBoss ? 8 : 4);
        ctx.font = '900 9px monospace';
        ctx.fillStyle = enemy.isBoss ? '#fed7aa' : '#facc15';
        ctx.fillText(`${coinDropFor(enemy, runtimeForDraw.wave)}🪙`, enemy.x, enemy.y - enemy.size - (enemy.isBoss ? 42 : 22));
      }

      const player = runtimeForDraw.player;
      drawPlayerFlushBar(ctx, player, runtimeForDraw, latestToiletRef.current);
      ...
      if (boss) {
        ...
        const barW = Math.min(440, width - 240);
        ...
        ctx.lineWidth = 2;
        ctx.beginPath();
        ctx.roundRect(x, y, barW, 40, 14);
        ctx.fill();
        ctx.stroke();
        ctx.fillStyle = '#7f1d1d';
        ctx.fillRect(x + 14, y + 22, barW - 28, 8);
        ctx.fillStyle = '#fb923c';
        ctx.fillRect(x + 14, y + 22, (barW - 28) * ratio, 8);
        ctx.fillStyle = '#fed7aa';
        ctx.font = '900 12px monospace';
        ctx.textAlign = 'center';
        ctx.fillText(`BOSS WAVE ${boss.bossWave}: ${boss.name}`, width / 2, y + 15);
        ctx.restore();
      }
  ...
  const sellOwnedToilet = (toilet: Toilet) => {
    if (toilet.id === STARTING_TOILET_ID || activeToilet.id === toilet.id) return;
    ...
    <div className="overflow-hidden rounded-[2rem] border border-slate-800 bg-slate-950/80 shadow-2xl shadow-black/30">
      <div className="flex flex-col gap-3 border-b border-slate-800 bg-slate-900/70 p-4 font-mono lg:flex-row lg:items-center lg:justify-between">
        <div>
          ...
          <h3 className="text-xl font-black text-white">Boss wave every 5 waves.</h3>
          <p className="mt-1 text-xs font-bold text-slate-400">Wave 5, 10, 15, and beyond spawn one boss. Defeat it for a big coin burst.</p>
        </div>
      ...
      <div className="grid gap-0 lg:grid-cols-[minmax(0,1fr)_330px]">
        <div ref={shellRef} className="relative min-h-[520px] overflow-hidden bg-slate-950">
          ...
          {gameState === 'playing' && (
            <button type="button" onClick={togglePause} className="absolute left-4 top-4 z-30 rounded-2xl border border-sky-300/35 bg-slate-950/80 px-4 py-2 font-mono text-xs font-black uppercase text-sky-100 shadow-xl backdrop-blur-md transition hover:bg-sky-400/15">
              {isPaused ? <Play className="mr-1 inline h-4 w-4" /> : <Pause className="mr-1 inline h-4 w-4" />}
              {isPaused ? 'Resume' : 'Pause'}
            </button>
          )}

          {bossName && gameState === 'playing' && !isPaused && (
            <div className="pointer-events-none absolute left-1/2 top-20 z-30 -translate-x-1/2 rounded-2xl border border-orange-300/40 bg-orange-500/20 px-5 py-3 text-center font-mono text-xs font-black uppercase text-orange-100 shadow-2xl backdrop-blur-md">
              Boss Appeared: {bossName}
            </div>
          )}
          ...
          {gameState !== 'playing' && (
            <div className="absolute inset-0 z-20 flex items-center justify-center bg-slate-950/75 p-5 text-center font-mono backdrop-blur-sm">
              <div className="w-full max-w-md rounded-3xl border border-amber-400/25 bg-slate-900/95 p-6 shadow-2xl">
                ...
                <p className="mt-2 text-sm font-bold text-slate-400">{gameState === 'gameover' ? `You scored ${score} and collected ${sessionCoins} coins this run.` : 'Every 5th wave brings a boss with a huge coin reward.'}</p>
                <div className="mt-5 grid gap-3 sm:grid-cols-2">
                  <button type="button" onClick={() => startGame('pc')} className="rounded-2xl bg-amber-400 px-5 py-4 text-sm font-black uppercase text-slate-950 shadow-xl shadow-amber-500/20 transition hover:bg-amber-300"><Play className="mr-2 inline h-4 w-4" /> Start PC Play</button>
                  <button type="button" onClick={() => startGame('mobile')} className="rounded-2xl border border-cyan-300/35 bg-cyan-400/15 px-5 py-4 text-sm font-black uppercase text-cyan-100 transition hover:bg-cyan-400/25"><Play className="mr-2 inline h-4 w-4" /> Start Mobile Play</button>
                </div>
              </div>
            </div>
          )}

          {controlMode === 'mobile' && gameState === 'playing' && !isPaused && (
            <div className="pointer-events-none absolute inset-x-0 bottom-0 z-30 flex items-end justify-between gap-4 p-5 font-mono">
              <div className="pointer-events-auto relative h-32 w-32 rounded-full border border-cyan-300/30 bg-slate-900/70 shadow-2xl backdrop-blur-md touch-none" onPointerDown={(event) => { event.currentTarget.setPointerCapture(event.pointerId); const rect = event.currentTarget.getBoundingClientRect(); joystickRef.current = { active: true, startX: rect.left + rect.width / 2, startY: rect.top + rect.height / 2, x: event.clientX, y: event.clientY }; }} onPointerMove={(event) => { if (!joystickRef.current.active) return; joystickRef.current.x = event.clientX; joystickRef.current.y = event.clientY; }} onPointerUp={() => { joystickRef.current.active = false; }} onPointerCancel={() => { joystickRef.current.active = false; }}>
              ...
              <button type="button" onPointerDown={(event) => { event.preventDefault(); triggerFlush(); }} className="pointer-events-auto h-28 w-28 rounded-full border border-amber-200/40 bg-amber-400 text-sm font-black uppercase text-slate-950 shadow-2xl shadow-amber-500/30 transition active:scale-95">Flush</button>
            </div>
          )}
        </div>
        ...
        <aside className="border-t border-slate-800 bg-slate-900/80 p-4 font-mono lg:border-l lg:border-t-0">
          <div className="mb-4 grid grid-cols-2 gap-2">
            ...
            <button type="button" onClick={togglePause} disabled={gameState !== 'playing'} className="rounded-xl border border-sky-400/25 bg-sky-400/10 px-3 py-3 text-xs font-black uppercase text-sky-200 transition hover:bg-sky-400/20 disabled:cursor-not-allowed disabled:opacity-40">{isPaused ? <Play className="mr-1 inline h-4 w-4" /> : <Pause className="mr-1 inline h-4 w-4" />} {isPaused ? 'Resume' : 'Pause'}</button>
            <button type="button" onClick={restartGame} className="rounded-xl border border-cyan-400/25 bg-cyan-400/10 px-3 py-3 text-xs font-black uppercase text-cyan-200 transition hover:bg-cyan-400/20"><RotateCcw className="mr-1 inline h-4 w-4" /> Restart</button>
          </div>

          <div className="rounded-2xl border border-slate-800 bg-slate-950 p-4">
            <div className="text-[10px] font-black uppercase tracking-[0.22em] text-slate-500">Equipped Toilet</div>
            <div className="mt-2 flex items-center gap-3"><div className="text-4xl">{activeToilet.emoji}</div><div><div className="text-sm font-black text-white">{activeToilet.name}</div><div className="mt-1 text-xs font-bold text-slate-400">Damage {activeToilet.damage} · Radius {activeToilet.flushRadius} · Cooldown {(activeToilet.cooldownMs / 1000).toFixed(1)}s</div></div></div>
            <div className="mt-3 h-2 overflow-hidden rounded-full bg-slate-800"><div className="h-full rounded-full bg-sky-400 transition-all" style={{ width: `${cooldownPercent * 100}%` }} /></div>
          </div>

          <div className="mt-4 rounded-2xl border border-orange-400/25 bg-orange-500/10 p-4 text-xs font-bold leading-relaxed text-orange-100">
            <div className="text-[10px] font-black uppercase tracking-[0.22em] text-orange-300">Boss Waves</div>
            <p className="mt-2">Bosses appear on waves 5, 10, 15, and beyond. Normal spawning pauses while a boss is alive. Bosses drop 18+ coins.</p>
          </div>
          ...
          <div className="mt-4 rounded-2xl border border-slate-800 bg-slate-950/70 p-4 text-xs font-bold leading-relaxed text-slate-400">
            <div className="text-[10px] font-black uppercase tracking-[0.22em] text-amber-300">Coin Drops</div>
            <div className="mt-2 grid gap-1"><span>🦠/🪰: 1 coin</span><span>🧼: 2 coins</span><span>🧻: 3 coins</span><span>🧹: 4+ coins</span><span className="text-orange-200">🚽 Bosses: 18+ coins</span></div>
          </div>
          ...
          {shopOpen && (
            <div className="mt-4 max-h-[440px] space-y-2 overflow-y-auto pr-1">
              {shopToilets.map((toilet) => {
                ...
                return <div key={toilet.id} className="rounded-2xl border border-slate-800 bg-slate-950/85 p-3"><div className="flex items-start justify-between gap-3"><div className="flex min-w-0 gap-3"><div className="text-3xl">{toilet.emoji}</div><div className="min-w-0"><div className="truncate text-xs font-black text-white">{toilet.name}</div><div className="mt-1 text-[10px] font-bold text-slate-500">Cost {toilet.cost} · DMG {toilet.damage} · R {toilet.flushRadius}</div></div></div><button type="button" disabled={!owned && !affordable} onClick={() => buyToilet(toilet)} className={`rounded-lg px-3 py-2 text-[10px] font-black uppercase transition ${active ? 'bg-emerald-400 text-slate-950' : owned ? 'bg-cyan-400/15 text-cyan-200 hover:bg-cyan-400/25' : affordable ? 'bg-amber-400 text-slate-950 hover:bg-amber-300' : 'cursor-not-allowed bg-slate-800 text-slate-500'}`}>{active ? 'Active' : owned ? 'Equip' : affordable ? 'Buy' : 'Need Coins'}</button></div>{owned && toilet.id !== STARTING_TOILET_ID && !active && <button type="button" onClick={() => sellOwnedToilet(toilet)} className="mt-2 text-[10px] font-black uppercase text-rose-300 hover:text-rose-200">Sell for {Math.floor(toilet.cost * 0.9)} coins</button>}</div>;
              })}
            </div>
          )}
          ...
          {!shopOpen && (
            <div className="mt-4 rounded-2xl border border-slate-800 bg-slate-950/70 p-4 text-xs font-bold leading-relaxed text-slate-400">
              ...
              <p className="mt-3">PC: WASD/Arrows + Space. Pause with P or Esc. Mobile: use the joystick and Flush button.</p>
            </div>
          )}
        </aside>
      </div>
